...
export async function POST(req: Request) {
  try {
    ...
    const {
      ...
      details = 'A visitor accessed RGStudio site.',
      scenario,
      result,
    } = body;
    ...
    const extraFields = [];
    if (scenario) extraFields.push(`Scenario: ${scenario}`);
    if (result) extraFields.push(`Result / Status: ${result}`);
    const extraText = extraFields.length > 0 ? `\n${extraFields.join('\n')}\n` : '';
...
`;
    ...
    const subject = `RGStudio Visitor Alert: ${event} (${locationInfo})`;

    const info = await transporter.sendMail({
      from: `"RGStudio Alerts" <${senderEmail}>`,
      to: receiverEmail,
      subject: subject,
      text: textBody,
    });
